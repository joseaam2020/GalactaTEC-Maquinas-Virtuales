...
from widgets.emailservice import is_valid_email
from register.bd import validate_password, update_player, get_player
...
class EditUser:
    def __init__(self, game):
        ...
        # Botones
        self.buttons_data = ["Apply", "Return"]
        # Creamos los botones
        self.buttons = []
   ...
   # =================== EVENTOS ===================
    ...
    def register(self):
        ...
        # Suponemos que guardaste los valores originales al cargar el jugador
        orig_username = getattr(self, "orig_username", "")
        orig_fullname = getattr(self, "orig_fullname", "")
        orig_email    = getattr(self, "orig_email", "")
        orig_music    = getattr(self, "orig_music", "")
        orig_ship     = getattr(self, "orig_ship", "")
        orig_photo    = getattr(self, "orig_photo", "")

        # Diccionario para llevar solo los cambios
        cambios = {}

        # Verificar cambio de username por separado
        if username != orig_username:
            if username_exists(username=username, db_path="./src/register/GalactaDB.db"):
                ...
                return
            cambios["new_username"] = username
        
        if fullname != orig_fullname:
            cambios["full_name"] = fullname
        if email != orig_email:
            cambios["email"] = email
        if password:
        ...
        if email in cambios:
            ...
            if not is_valid_email(email):
                self.show_error("El correo no es válido, por favor ingrese otro.")
                return
        ...
        if cambios:
            ...
            if success:
                # Actualiza logged_users si el username cambió
                if "new_username" in cambios:
                    main_window.logged_users.discard(orig_username)  # Eliminar el viejo
                    main_window.logged_users.add(cambios["new_username"])  # Agregar el nuevo
                    
                    # Actualiza el diccionario de players
                    self.game.players[cambios["new_username"]] = self.game.players.pop(orig_username)
                    
                    # Actualiza el diccionario de patterns
                    self.game.patterns[cambios["new_username"]] = self.game.patterns.pop(orig_username)
                    
                    # Actualiza el jugador activo en Options
                    if hasattr(self.game.states["OPTIONS"], "active_player"):
                        if self.game.states["OPTIONS"].active_player == orig_username:
                            self.game.states["OPTIONS"].active_player = cambios["new_username"]
                    
                    # Actualiza current_player si es el mismo
                    if self.game.current_player == orig_username:
                        self.game.current_player = cambios["new_username"]

                # Actualiza la información del jugador en el diccionario
                username_final = cambios.get("new_username", orig_username)
                updated_info = get_player(username=username_final, db_path="./src/register/GalactaDB.db")
                if updated_info:
                    self.game.players[username_final] = updated_info

                self.show_error("Datos actualizados correctamente", duration=2000)
                self.game.change_state("OPTIONS")
        else:
            self.show_error("No se detectaron cambios para actualizar.")
    ...
    def load_player_data(self, username):
        ...
        result = get_player(username,  db_path="./src/register/GalactaDB.db")
        if result:
            print("Result: ", result )
            photo_path = result["photo_path"] 
            ship_image = result["ship_image"] 
            music_pref = result["music_pref"]
                
            # Colocar los valores en los campos de texto
            self.username_field.update_text(username)
            self.fullname_field.update_text(result["full_name"])
            self.email_field.update_text(result["email"])
            self.password_field.update_text("")

            # Colocar las rutas en las variables de selección de archivos
            self.selected_profile_pic = photo_path if photo_path else "No seleccionada"
            self.selected_music = music_pref if music_pref else "No seleccionada"
            self.selected_ship = ship_image if ship_image else ""

            # ✅ AGREGAR: Guardar valores originales para comparación
            self.orig_username = username
            self.orig_fullname = result["full_name"]
            self.orig_email = result["email"]
            self.orig_photo = self.selected_profile_pic
            self.orig_music = self.selected_music
            self.orig_ship = self.selected_ship

            # Resaltar la nave seleccionada
            for i in range(1, 4):
                print(self.selected_ship)
                self.buttons[3 + i].highlighted = self.selected_ship.endswith(f"player_ship_{i}.png")
                
            print(f"Datos del jugador '{username}' cargados correctamente.")
            return True
        else:
            print(f"No se encontró ningún jugador con username '{username}'.")
            return False
